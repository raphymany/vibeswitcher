...
| Audit follow-ups | AF1 — custom switch-sound files are copied into a managed `Sounds` folder and import drops any path outside it (mirrors icon handling). AF2 — App-Trigger filter chips are keyboard-accessible (focusable + Enter/Space + automation names). AF3–AF5 logged in BACKLOG (CI pins need a workflow run; accent-frame de-dup is runtime-only verifiable) | ✅ Done (AF1/AF2) |
| Tests | 8 audit tests + final-pass tests (import-guard reject, reset-to-defaults preservation, log-path) + AF1 sound-confinement ×2; suite at 237 | ✅ Done |
