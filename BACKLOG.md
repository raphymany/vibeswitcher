...
| ~~AF4~~ | XAML de-duplication | ✅ Done where clean (accent frame → one shared `Views/AccentFrame.xaml` across 19 dialogs). Remaining items investigated and **closed as won't-do**: `MuteBannerToggle` bakes its own icon (not a clean `BasedOn`); the two `DayPillStyle` copies actually differ in size/padding/corner-radius (merging would change one's look); the orange accent values are inline trigger setters (magic-number promotion, high churn). All visual-only, no functional benefit, unverifiable without running the UI. |
